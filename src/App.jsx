...
// ══════════════════════════════════════════════════════════════════════════════
// IALAB DRIVE STORAGE — Guarda en Google Drive del dueño del sistema
// Cada instalación tiene un ID único de cliente
// ══════════════════════════════════════════════════════════════════════════════
const DRIVE_URL    = import.meta.env.VITE_DRIVE_URL || '';    // URL del Apps Script
const CLIENTE_ID   = import.meta.env.VITE_CLIENTE_ID || 'CV_DEMO';   // ID único por cliente
const CLIENTE_NOMBRE = import.meta.env.VITE_CLIENTE_NOMBRE || 'Click Vision Demo';

const driveStorage = {
  async guardar(tabla, registros) {
    if (!DRIVE_URL) {
      // Sin Drive configurado: usa localStorage como respaldo
      try { localStorage.setItem('cv-' + tabla, JSON.stringify(registros)); } catch(_){}
      return;
    }
    try {
      ...
      // También guarda local como cache rápido
      try { localStorage.setItem('cv-' + tabla, JSON.stringify(registros)); } catch(_){}
    } catch(e) {
      // Si falla Drive, al menos guarda local
      try { localStorage.setItem('cv-' + tabla, JSON.stringify(registros)); } catch(_){}
    }
  },
  ...
  async cargar(tabla) {
    if (!DRIVE_URL) {
      // Sin Drive: carga desde localStorage
      try {
        const local = localStorage.getItem('cv-' + tabla);
    ...
    try {
      const res = await fetch(DRIVE_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'text/plain' },
        body: JSON.stringify({ accion: 'cargar', cliente_id: CLIENTE_ID, tabla })
      });
      const data = await res.json();
      if (data.ok && data.datos && data.datos.length > 0) {
        // Actualiza cache local también
        try { localStorage.setItem('cv-' + tabla, JSON.stringify(data.datos)); } catch(_){}
        return data.datos;
      }
    } catch(_) {}
    // Si Drive falla, intenta cargar desde localStorage
    try {
      const local = localStorage.getItem('cv-' + tabla);
      return local ? JSON.parse(local) : null;
    } catch { return null; }
...
export default function ClickVisionSystem() {
  ...
  const bootStorage = async () => {
    try {
      const p  = await driveStorage.cargar('patients');   if(p)  setPatients(p);
      const pr = await driveStorage.cargar('products');   if(pr) setProducts(pr);
      const a  = await driveStorage.cargar('agenda');     if(a)  setAgenda(a);
      const c  = await driveStorage.cargar('campaigns');  if(c)  setCampaigns(c);
    } catch(_){}
  };

  const save = async (k,v) => { await driveStorage.guardar(k.replace('cv-',''), v); };
