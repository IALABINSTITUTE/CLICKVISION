...
export default function ClickVisionSystem() {
  ...
  const bootStorage = () => {
    try {
      const p = localStorage.getItem('cv-patients'); if(p) setPatients(JSON.parse(p));
      const pr= localStorage.getItem('cv-products'); if(pr) setProducts(JSON.parse(pr));
      const a = localStorage.getItem('cv-agenda');   if(a) setAgenda(JSON.parse(a));
      const c = localStorage.getItem('cv-campaigns');if(c) setCampaigns(JSON.parse(c));
    } catch(_){}
  };
